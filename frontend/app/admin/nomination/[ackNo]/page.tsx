...
export default function NominationDetailPage() {
  ...
  const handleDeleteDocument = async (documentType: string, s3Key: string) => {
    ...
    try {
      ...
      await api.delete(
        `/nomination/${nomination._id}/documents/${documentType}?s3Key=${encodeURIComponent(s3Key)}`
      );
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
              ...
              <div className="px-6 py-4 space-y-3">
                ...
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
                  ...
                  {nomination.maintenanceReceiptsDocuments &&
                  nomination.maintenanceReceiptsDocuments.length > 0 ? (
                    <div className="space-y-2">
                      {nomination.maintenanceReceiptsDocuments.map(
                        (doc, idx) => (
                          <div
                            key={doc.s3Key}
